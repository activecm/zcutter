...
def process_log_lines(log_file, original_filename, requested_fields, cl_args):
	...
	field_line_fields = []
	type_line_fields = []
	field_types = {}										#Dictionary whose keys are ALL field names and whose values are the corresponding field types.  This has all the field names, even when we limit the output to only certain fields.

	field_location = {}								#Remembers in which column we can find a given field name.
...
def relative_touch(old_file, new_file_to_change):
